...
- (SYFaceImage *) faceImageFromSampleBuffer:(CMSampleBufferRef) sampleBuffer
{
    //获取灰度图像数据
    ...
    SYFaceImage* faceImage=[[SYFaceImage alloc] init];
    if(!faceImage){
        return nil;
    }
...
//    faceImage.data = (__bridge_transfer NSData*)CGDataProviderCopyData(provider);
    ...
    faceImage.data  = UIImageJPEGRepresentation(image, 1);
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(onOutputFaceImage:)]){
        SYFaceImage* faceImage=[self faceImageFromSampleBuffer:sampleBuffer];
...
-(IFlyFaceDirectionType)faceImageOrientation
{
    ...
    switch (self.interfaceOrientation) {
        case UIDeviceOrientationPortrait:{
            //IFlyFaceDirectionTypeLeft
            faceOrientation=IFlyFaceDirectionTypeUp;
        }
            break;
        case UIDeviceOrientationPortraitUpsideDown:{
            //IFlyFaceDirectionTypeRight
            faceOrientation=IFlyFaceDirectionTypeDown;
        }
            break;
